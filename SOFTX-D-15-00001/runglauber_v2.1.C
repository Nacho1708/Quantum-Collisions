...
//---------------------------------------------------------------------------------
void plotNpart(const char* filename = "glauber_output.root", const char* object = "nt_Au_Au");
                      ...
                      const char *fname)
{
  ...
  //Modifications to save files of data
  // TFile *file = new TFile(outfile, "RECREATE");
  // TTree *tree = glauber.GetTree();
  // tree->Write();
  // file->Close();
  //
...
// NEW FUNCTION (Added by I. Quiroz)
void plotNpart(const char* filename = "glauber_output.root", const char* object = "nt_Au_Au") {
    // Open the ROOT file
    ...
    // Get the TTree
    TTree *tree = (TTree*)file->Get(object);
    if (!tree) {
    ...
    TCanvas *c1 = new TCanvas("c1", "Npart Distribution", 800, 600);

    printf("LOL1\n");
    // Draw the Npart histogram
    tree->Draw("Npart>>h1(100,0,400)");
    printf("LOL2\n");
    TH1F *h1 = (TH1F*)gDirectory->Get("h1");
    h1->SetTitle("Number of Participants;Npart;Counts");
    h1->Draw();

    // Save the plot as an image file
    c1->SaveAs("Npart_distribution.png");
